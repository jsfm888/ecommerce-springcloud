package com.njust.ecommerce.service.hystrix.request_merge;
 ...
 * */
@Slf4j
public class NacosClientCollapseCommand extends
        HystrixCollapser<List<List<ServiceInstance>>, List<ServiceInstance>, String>{
    ...
    protected void mapResponseToRequests(List<List<ServiceInstance>> batchResponse,
        ...
        for (CollapsedRequest<List<ServiceInstance>, String> collapsedRequest : collapsedRequests) {
            //从批量响应集合中按顺序取出结果
            List<ServiceInstance> response = batchResponse.get(count++);
            collapsedRequest.setResponse(response);
        }
